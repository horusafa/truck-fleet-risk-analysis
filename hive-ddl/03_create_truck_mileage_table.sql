-- Create truck_mileage table from trucks table
-- Transforms wide format (monthly columns) to long format (rows per month)
-- Calculates MPG (miles per gallon) for each truck per month

CREATE TABLE truck_mileage AS 
SELECT truckid, driverid, rdate, miles, gas, miles / gas AS mpg 
FROM trucks 
